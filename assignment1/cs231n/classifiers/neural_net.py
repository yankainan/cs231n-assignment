...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    hidden = np.maximum(0, np.dot(X, W1) + b1) #! 注意这里包含了ReLU激活函数的计算
    scores = np.dot(hidden, W2) + b2 #*计算最终得分
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    dscore = np.exp(f) / s.reshape(N,1) #! 这里是计算最右边的dscore的梯度
    dscore[range(N), y] -= 1
    dscore /= N
    
    grads['W2'] = np.dot(hidden.T, dscore) + 2 * reg * W2
    grads['b2'] = np.sum(dscore, axis=0) #! 这里我觉得比较难理解，可以想做是每一个样本分开求导，然后在 db 是累加的，所以相当于dscore直接按行相加
    
    dhidden = np.dot(dscore, W2.T)
    dhidden[hidden == 0] = 0 #! 注意在进行hidden计算的时候，使用了ReLU函数的计算，因此在小于零的部分其实hidden已经是0了，没有梯度了
    
    grads['W1'] = np.dot(X.T, dhidden) + 2 * reg * W1 #! 别忘了加上正则
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):#* 使用SGD进行优化
      X_batch = None
      y_batch = None
