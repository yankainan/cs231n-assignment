...
#* 在把我们的输入送给RNN之前，我们还需要做一个额外的操作，那就是word embedding，什么是word embedding呢？简单来说就是将一个单词表示成一个向量的形式，因为我们神经网络要训练，总不能把一个单词直接扔进去训练吧？所以我们要把我们扔进去的单词先披个皮衣，转换成常见的矩阵形式，再丢给神经网络。这里需要实现的是word_embedding_forward和word_embedding_backward两个函数。
#* forward函数里给我们提供了一个转换矩阵W，这个矩阵囊括了我们想要用到的词库，也就是说这个矩阵的每一行对应一个单词的向量表示，因此在做embedding的时候我们利用numpy的index机制，从这个矩阵中取出我们要丢进网络的这些单词的向量表示。做backward的时候用到了np.add.at函数，这个函数的作用是将numpy矩阵中相应的位置加上相应的值，这里np.add.at(dW, x, dout)的意思就是把dout的值按照其出现的位置x加到dW对应的位置上。
