...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
          ...
          dlstm, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dtemporal, lstm_cache)
          grads['W_embed'] = word_embedding_backward(dlstm, emb_in_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'lstm':
          prev_c = np.zeros_like(prev_h) #! cell state 和 hidden state 的维度是一样的
        x = np.array([self._start for i in range(N)]) #! 每一张图片开头都是<start>，下一行同理，但是变量 captions 和 x 不同的是 captions 表示的是这些图片的标题，而 x 只表示预测这些图片标题的下一个单词
        captions[:, 0] = self._start
        #* (2)
        for t in range(1, max_length):
          x_emb, _ = word_embedding_forward(x, W_embed)
          if self.cell_type == 'rnn':
            next_h, next_cache = rnn_step_forward(x_emb, prev_h, Wx, Wh, b)
            prev_h = next_h
          elif self.cell_type == 'lstm':
